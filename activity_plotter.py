...
class activityPlotter:
    ...
    def plot_cumulative_miles_year(self, year=2019, annotate=True, show=True,\
        ...
        if not any(year_filter):
            raise ValueError('No activities were found for year %i' % year)
        ...
        plt.plot(plot_days, cumulative_dist,\
                 label=str(year))
        if annotate:
...
if __name__ == '__main__':
    ...
    activity_plotter.plot_cumulative_miles(ax, save_file='cumulative_miles.png')
